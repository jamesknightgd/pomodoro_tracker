...
*/
export default async function handleRequestEachWeeksWorkTime(app: express.Express)
{
    app.get("/api/EachWeeksWorkTime", async (request, response)=>
    {
        ...
        const weekStartToTotalWorkedTimeInWeek : {[key : string] : HoursMinutesSeconds} = {}
        for(const weekStart in weekStartToTotalSecondsWorkedInWeek)
        {
            weekStartToTotalWorkedTimeInWeek[weekStart] = cHoursMinutesSecondsFromSeconds(weekStartToTotalSecondsWorkedInWeek[weekStart])
        }

        response.type(`application/json`).status(200).send(weekStartToTotalWorkedTimeInWeek)
    })
}